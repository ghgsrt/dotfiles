...
setup_system() {
	...
	# Ensure /etc/zlogin does not already exist
	if [ -e "/etc/zlogin" ]; then
		echo "Warning: /etc/zlogin already exists. Please remove it to replace it."
		return
	fi
	...
	# Generate system zlogin for default environment variables
	cat << EOF | should_sudo tee "${DOTFILES_DIR}/generated/system-zlogin" > /dev/null
